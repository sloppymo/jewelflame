...
# Skeleton - NON-STANDARD sprite sheet layout
# Layout: Attack01(0), Attack02(1), Block(2), Death(3), Hurt(4), Idle(5), Walk(6)

func _ready():
	# Override the standard sprite frame building
	if source_texture:
		build_skeleton_frames()
	else:
...
func build_skeleton_frames():
	"""Build SpriteFrames for Skeleton's layout.
	
	Skeleton layout (7 rows) - ACTUAL ORDER IN SHEET:
	Row 0: Idle (6 frames)
	Row 1: Walk (8 frames)
	Row 2: Attack01 (6 frames)
	Row 3: Attack02 (7 frames)
	Row 4: Hurt (4 frames)
...
func _die():
	...
	if sprite.sprite_frames and sprite.sprite_frames.has_animation("death"):
		...
		push_warning(name + ": Death animation missing!")
		_change_state(State.DEAD)
