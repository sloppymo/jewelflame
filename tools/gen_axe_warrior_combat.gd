...
# Axe Warrior Combat SpriteFrames Generator
# Frame size: 32x32, 4-directional (down/up/right/left)
# 20 rows = 5 animations (idle + 4 attacks + hurt)

func _run() -> void:
	print("Generating Axe Warrior Combat SpriteFrames...")
	
	var sprite_frames := SpriteFrames.new()
	
	var texture: Texture2D = load("res://assets/Citizens-Guards-Warriors/AxeWarrior/axe_warrior_combat_fx.png")
	if not texture:
		print("ERROR: Could not load axe warrior combat texture!")
		return
	...
	var output_path := "res://assets/animations/axe_warrior_combat.tres"
