...
# Rogue (Regular) Non-Combat Bow SpriteFrames Generator
# Frame size: 16x16
# Directions: right, left, down, up (4 rows per animation group)
# Source: assets/Citizens-Guards-Warriors/Rogues/Hooded/rogue_hooded_nc_bow.png

func _run() -> void:
	print("Generating Hooded Rogue Non-Combat Bow SpriteFrames...")
	
	var sprite_frames := SpriteFrames.new()
	
	var texture: Texture2D = load("res://assets/Citizens-Guards-Warriors/Rogues/Hooded/rogue_hooded_nc_bow.png")
	if not texture:
		print("ERROR: Could not load rogue bow texture!")
		return
	...
	var output_path := "res://assets/animations/rogue_hooded_nc_bow.tres"
