...
# Axe Warrior Non-Combat SpriteFrames Generator
# Frame size: 16x16, 4-directional (down/up/right/left)
# 31 rows = 9 animation groups

func _run() -> void:
	print("Generating Axe Warrior Non-Combat SpriteFrames...")
	
	var sprite_frames := SpriteFrames.new()
	
	var texture: Texture2D = load("res://assets/Citizens-Guards-Warriors/AxeWarrior/axe_warrior_nc.png")
	if not texture:
		print("ERROR: Could not load axe warrior texture!")
		return
	...
	var output_path := "res://assets/animations/axe_warrior_non_combat.tres"
