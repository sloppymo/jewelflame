...
# Caped Warrior Combat SpriteFrames Generator
# Frame size: 32x32, 4-directional (down/up/right/left)
# 20 rows = 5 animations

func _run() -> void:
	print("Generating Caped Warrior Combat SpriteFrames...")
	
	var sprite_frames := SpriteFrames.new()
	
	var texture: Texture2D = load("res://assets/Citizens-Guards-Warriors/CapedWarrior/caped_warrior_combat.png")
	if not texture:
		print("ERROR: Could not load caped warrior combat texture!")
		return
	...
	var output_path := "res://assets/animations/caped_warrior_combat.tres"
